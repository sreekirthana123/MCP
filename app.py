...
def fetch_gmail_threads() -> list[dict[str, Any]]:
    ...
    raw = fetch_threads(max_results=20)
...
def render_inbox_phase() -> None:
    ...
    if needs_reply_threads:
        st.info(f"{len(needs_reply_threads)} threads need a reply → go to **Draft Generation**")
                ...
                if status == "approved":
                    ...
                    if st.session_state.get(f"editing_{thread_id}"):
                        ...
                        edited_text = st.text_area(
                            ...
                            value=draft_data.get("draft", ""),
                            height=300,
                            key=f"edited_draft_area_{thread_id}",
                        )
                        if st.button("✅ Approve Edited Version", type="primary", key=f"approve_edit_btn_{thread_id}"):
                            st.session_state.approved[thread_id] = {
                                "draft": edited_text,
                                "thread": thread,
                                "metadata": draft_data,
