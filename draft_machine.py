...
4. STRUCTURE
    ALL FIVE parts below are MANDATORY. Never skip any of them.
    Output is incomplete if any part is missing.

    (a) greeting line: "Hi [Name],"
    (b) one-line warm acknowledgment referencing the specific message
    (c) the response / substance
    (d) ONE clear next step or question — this line is REQUIRED
    (e) sign-off: copy EXACTLY from "Default signature to use:" 
        in the system instructions — never infer from the thread.
        Must include all three lines: closing word, name, title.

    No preamble. No extra commentary. Just the email body, ready to send.
"""
